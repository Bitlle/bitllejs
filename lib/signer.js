...
var Signer = function (bitllejs) {
    ...
    this._getDataToSig = function (token, to, amount, fee, nonce) {
        ...
        return this.web3.sha3(correctParam, {encoding: 'hex'});
    };
...
Signer.prototype.getOrderSign = function (sell, buy, price, amount, cell, fee, nonce, privateKey) {
    ...
    if (!this.web3.isAddress(buy)) {
        throw "ERROR in GenerateOrderTX() _tknBuy address:" + buy + " incorrect!";
    }
    if (!this.web3.isAddress(sell)) {
        throw "ERROR in GenerateOrderTX() _tknSell address:" + sell + " incorrect!";
    }
...
Signer.prototype.getDealSign = function (sell, buy, amount, cell, fee, nonce, privateKey) {
    ...
    if (!this.web3.isAddress(buy)) {
        throw "ERROR in GenerateDealTX() _tknBuy address:" + buy + " incorrect!";
    }
    if (!this.web3.isAddress(sell)) {
        throw "ERROR in GenerateDealTX() _tknSell address:" + sell + " incorrect!";
    }
