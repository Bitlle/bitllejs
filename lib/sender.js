...
var Sender = function (bitllejs) {
    this.web3 = bitllejs.web3;
};

Sender.prototype.send = function (to, _msgData, _nonce, _pk) {
    console.log("\nSending TX");
    ...
    if (0 < tx_estimateGas) {
        if (3000000 > tx_estimateGas) {
            ...
            var tx_gasPrice = (4000000000 > av_gasPrice ? this.web3.toHex(av_gasPrice) : 4000000000);
            ...
            var tx = new Tx(rawTx);
            tx.sign(new Buffer(_pk.slice(2), 'hex'));
