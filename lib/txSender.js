                        ...
                        function checkCur() {
                            ...
                            web3.eth.getTransaction(curTx, (err, tinfo) => {
                                if (err) {
                                    ...
                                    if (params.txPool[curTxIndex].nonce && params.txPool[curTxIndex].nonce < nonce) {
                                        let msg = "WATCH TX: " + curTx + " tx has been replaced (mined) by another tx with same nonce, monitor stop";
                                        if (log) console.log(msg);
                                        reject(msg);
                                    }
                                    else if (Date.now() > (params.txPool[curTxIndex].timestamp + params.lossTimeOut)) {
                                        let msg = "WATCH TX: " + curTx + " tx has been lost, monitor stop by timeout " + params.lossTimeOut / 1000 + ' sec';
                                        if (log) console.log(msg);
                                        reject(msg);
                                    }
                                    else if (curTxIndex-- > 0) {
                                        checkCur(); // next tx
                                    }
                                    else {
                                    ...
                                    params.txPool[curTxIndex].nonce = tinfo.nonce;
                                    ...
                                    // check(isRun = false); //to new cycle
                                    // }
                                    if (log) console.log('WATCH TX: mined with tx hash', curTx);
                                    resolve(tinfo);
                                }
                            });
                        }
