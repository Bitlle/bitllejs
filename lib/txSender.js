...
TxSender.prototype.boostTx = function (params, callBack) {
    ...
    let result = new Promise(function (resolve, reject) {
        ...
        let rawTx = {
            ...
            gasPrice: params.txInfo.gasPrice.plus(params.step).toNumber(),
            gasLimit: params.txInfo.gas,
            to: params.txInfo.to,
            value: params.txInfo.value.toString(),
...
/**
 * returns the answer when the transaction is completed, automatically increases the price of gas, if necessary, by replacing the transaction.
 * @param {object} params 
 * {
...
TxSender.prototype.watchTx = function (params, callBack) {
    ...
    let result = new Promise(function (resolve, reject) {
        ...
        if (!params.updateTxHash) params.updateTxHash = () => { };
                ...
                function checkCur() {
                    ...
                    web3.eth.getTransaction(curTx, (err, tinfo) => {
                        if (err) {
                            ...
                            if (log && curTxIndex == params.txPool.length - 1) console.log("'WATCH TX: " + curTx + " not found yet");
                            if (curTxIndex-- > 0) checkCur(); // next tx
                            else check(); //to new cycle
